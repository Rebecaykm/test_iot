...
class HourlyProductionGraph extends Component
{
    ...
    public function getHourlyProductionGraph()
    {
        ...
        foreach ($historyRecords as $record) {
            ...
            $groupedData[$record->production_id][$record->work_name][$record->part_number]['planned_quantity'] = $record->planned_quantity;
            $groupedData[$record->production_id][$record->work_name][$record->part_number]['production_per_hour'][$hourKey] =
                ($groupedData[$record->production_id][$record->work_name][$record->part_number]['production_per_hour'][$hourKey] ?? 0) + $record->quantity;
        }

        // Preparar datos para Chart.js
        $hours = $startDateTime->diffInHours($endDateTime);
        foreach ($groupedData as $productionId => $prodctions) {
            foreach ($prodctions as $workName => $parts) {
