...
class RecoverProductionOrderNumbersLive implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if (!$lock->get()) {
            $this->log()->warning('RecoverProductionOrderNumbersLive: ya hay una recuperación en curso, se omite esta ejecución');
            return;
        }
        ...
        try {
            ...
            $this->log()->error('FALLO CRÍTICO en RecoverProductionOrderNumbersLive: ' . $e->getMessage());
            throw $e;
        } finally {
            $lock->release();
        }
    ...
    {
        if ($recoveredCount > 0) {
            $this->log()->info("RecoverProductionOrderNumbersLive recuperó $recoveredCount número(s) de orden");
        }

        if ($errorCount > 0) {
            $this->log()->warning("RecoverProductionOrderNumbersLive terminó con $errorCount errores. Detalle: " . implode(', ', $errors));
        }
    }
    ...
    {
        $this->log()->error('El Job RecoverProductionOrderNumbersLive ha fallado definitivamente: ' . $exception->getMessage());
    }
}
