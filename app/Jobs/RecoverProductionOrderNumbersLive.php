...
class RecoverProductionOrderNumbersLive implements ShouldQueue
{
    ...
    {
        $lock = Cache::lock('recover-production-order-numbers-live', $this->timeout + 30);

        if (!$lock->get()) {
            $this->log()->warning('[OMITIDO] RecoverProductionOrderNumbersLive: ya hay una recuperación en curso, se omite esta ejecución');
            return;
        }

        $this->log()->info('[INICIO] RecoverProductionOrderNumbersLive', [
            'Job' => 'RecoverProductionOrderNumbersLive',
            'Ambiente' => 'Live',
        ...
        try {
            ...
            $this->log()->info("[YF013 - INSERT] RecoverProductionOrderNumbersLive: $probesSent sonda(s) nueva(s) insertada(s) en YF013");
            ...
            foreach ($productionRecords as $record) {
                try {
                    ...
                    $this->log()->info('[ORDEN RECUPERADA] RecoverProductionOrderNumbersLive: número de orden recuperado', [
            ...
            $this->log()->error('[FALLO] FALLO CRÍTICO en RecoverProductionOrderNumbersLive: ' . $e->getMessage());
            throw $e;
        } finally {
            $lock->release();
        }
    ...
    protected function fetchOrderNumberFromFso($record, string $plannedDateFormatted): ?string
    {
        ...
        $this->log()->info('[FSO - CONSULTA] RecoverProductionOrderNumbersLive: registro encontrado en FSO', [
    ...
    {
        $this->log()->info("[RESULTADO] RecoverProductionOrderNumbersLive: $recoveredCount número(s) de orden recuperado(s), $errorCount error(es)");

        if ($errorCount > 0) {
            $this->log()->warning("[RESULTADO] RecoverProductionOrderNumbersLive terminó con $errorCount errores. Detalle: " . implode(', ', $errors));
        }
    }
    ...
    {
        $this->log()->error('[FALLO] El Job RecoverProductionOrderNumbersLive ha fallado definitivamente: ' . $exception->getMessage());
    }
}
