    ...
    <div class="card border-0 shadow-sm rounded-3 overflow-hidden">
        ...
        <div class="card-header bg-white border-0 py-3">
            <div class="d-flex justify-content-between align-items-center">
                    ...
                    <form method="GET" action="{{ route('lines.index') }}">
                        <div class="input-group">
                            ...
                            @if (!empty(request('search')))
                                <a href="{{ route('lines.index') }}" class="input-group-text bg-white border-start-0">
                ...
                <!-- Botón Agregar -->
                @can('create lines')
                    <a href="{{ route('lines.create') }}" class="btn btn-primary rounded-3">
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-light sticky-top">
                        <tr>
                            ...
                            <th class="fw-bold text-secondary text-uppercase border-light-subtle">{{ __('Color') }}</th>
                    ...
                    <tbody>
                        @forelse ($lines as $line)
                            <tr class="border-light-subtle">
                                <!-- Nombre -->
                                <td class="py-3 fw-500">{{ $line->name }}</td>

                                <!-- Descripción -->
                                <td class="py-3">
                                    <span class="text-muted small">{{ $line->description ?? '-' }}</span>
                                </td>
                                ...
                                <td class="py-3">
                                    @if ($line->color)
                                        <div class="d-flex align-items-center">
                                            <div class="color-indicator me-2" style="background-color: {{ $line->color }}; width: 20px; height: 20px; border-radius: 4px;"></div>
                                            <span class="small">{{ $line->color }}</span>
                                        </div>
                                    @else
                                ...
                                <!-- Fecha de Creación -->
                                <td class="py-3 small">
                                    <div class="fw-500">{{ optional($line->created_at)->format('Y-m-d') }}</div>
                                    <div class="text-muted">{{ optional($line->created_at)->format('H:i') }}</div>
                                </td>
                                ...
                                <td class="py-3 text-center">
                                    <div class="d-flex justify-content-center gap-2">
                                        @can('edit lines')
                                            <a href="{{ route('lines.edit', $line->id) }}" class="btn btn-sm btn-outline-primary rounded-3">
                                        ...
                                        @can('delete lines')
                                            <form action="{{ route('lines.destroy', $line->id) }}" method="POST" style="display:inline;" class="delete-form">
                                                @csrf
                                                @method('DELETE')
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-4">
                                    <div class="d-flex flex-column align-items-center">
                                        ...
                                        @if(request('search'))
                                            <a href="{{ route('lines.index') }}" class="btn btn-sm btn-link mt-2">
                                                Limpiar búsqueda
                                            </a>
                                        @endif
                                    </div>
            ...
            <div class="card-footer bg-white border-0 py-3">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="text-muted small">
                        Mostrando {{ $lines->firstItem() ?? 0 }} a {{ $lines->lastItem() ?? 0 }} de
                        {{ $lines->total() }} resultados
                    </div>
            ...
            document.querySelectorAll('.delete-form').forEach(form => {
                form.addEventListener('submit', function(e) {
                    ...
                    if (typeof Swal !== 'undefined') {
                        ...
                        if (confirm(
                                '¿Estás seguro de que deseas eliminar esta línea? Esta acción no se puede deshacer.'
                            )) {
                            this.submit();
                        }
                    }
