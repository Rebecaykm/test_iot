...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
        <div>
            <h1 class="m-0 font-weight-bold text-dark" style="font-size: 1.4rem;">Roles</h1>
        </div>

        <div class="d-flex gap-2">
            @can('create roles')
                <a href="{{ route('roles.create') }}" class="btn-action btn-action-primary"
                    aria-label="Agregar nuevo rol">
                    <i class="fas fa-plus" aria-hidden="true"></i>
                    <span class="d-none d-md-inline">Agregar Rol</span>
                </a>
            @endcan
        </div>
    </div>
            ...
            <form method="GET" action="{{ route('roles.index') }}">
                <div class="d-flex flex-wrap align-items-center justify-content-end gap-2">
                    ...
                    <div class="filter-group" style="width: 280px;">
                        <label for="role-search" class="sr-only">Buscar rol</label>
                        <i class="fas fa-search filter-icon" aria-hidden="true"></i>
                        <input type="text" id="role-search" name="search" class="filter-input"
                            placeholder="Buscar rol..." value="{{ request('search') }}">
                    </div>
                ...
                <table class="table table-hover mb-0">
                    <thead>
                        <tr class="table-head-row">
                            ...
                            <th class="th-cell" scope="col">Permisos</th>
                    ...
                    <tbody>
                        @forelse ($roles as $role)
                            <tr class="td-row">
                                <td class="td-cell">
                                    <span class="fw-600 text-dark" style="font-size: 0.85rem;">{{ $role->name }}</span>
                                </td>

                                <td class="td-cell">
                                    <div class="d-flex flex-wrap gap-1 mb-1">
                                        @foreach ($role->permissions->take(3) as $permission)
                                            <span class="badge-soft badge-primary">{{ $permission->name }}</span>
                                        @endforeach
                                ...
                                <td class="td-cell">
                                    <div class="fw-600" style="font-size: 0.82rem; color: #334155;">
                                        {{ optional($role->created_at)->format('Y-m-d') }}
                                    </div>
                                    <div class="text-muted" style="font-size: 0.75rem;">
                                        {{ optional($role->created_at)->format('H:i') }}
                                    </div>
                                </td>
                            ...
                            <tr>
                                <td colspan="4" class="text-center py-5">
                                    <div class="text-muted">
                                        <i class="fas fa-user-shield fa-3x mb-3 d-block" style="color: #cbd5e1;"></i>
            ...
            <div class="card-footer bg-white py-3" style="border-top: 1px solid #e9ecef;">
                <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <small class="text-muted">
                        Mostrando <strong>{{ $roles->firstItem() ?? 0 }}</strong> –
                        <strong>{{ $roles->lastItem() ?? 0 }}</strong> de
                        <strong>{{ $roles->total() }}</strong> registros
                    </small>
                    @if ($roles->hasPages())
...
@section('js')
    ...
    @include('partials.theme-scripts', ['deleteMessage' => '¿Estás seguro de que deseas eliminar este rol? Esta acción no se puede deshacer.'])
@stop
