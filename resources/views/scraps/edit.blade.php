...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
        <div>
            <h1 class="m-0 font-weight-bold text-dark" style="font-size: 1.4rem;">Editar Scrap</h1>
        </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
        <div class="card-header bg-white py-3" style="border-bottom: 1px solid #e9ecef;">
            <h5 class="mb-0 section-title">
                <i class="fas fa-recycle mr-2" style="color: #94a3b8;"></i>Información del Scrap
            </h5>
        </div>
        <div class="card-body p-4">
            <form action="{{ route('scraps.update', $scrap->id) }}" method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-md-6 mb-3 mb-md-0">
                        ...
                        <input type="text" name="code" id="code"
                            ...
                            value="{{ old('code', $scrap->code) }}" required placeholder="Ej: SCR-001">
                        @error('code')
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name" id="name"
                            ...
                            value="{{ old('name', $scrap->name) }}" required placeholder="Ej: Scrap de Plástico ABS">
                        @error('name')
                            <div class="field-error">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        @php $currentTypeScrapId = old('type_scrap_id', $scrap->type_scrap_id); @endphp
                        <x-select
                            name="type_scrap_id"
                            label="Tipo de Scrap *"
                            :options="collect([['value' => '', 'label' => 'Seleccione un tipo', 'selected' => !$currentTypeScrapId]])
                                ->concat($typeScraps->map(fn ($type) => [
                                    'value' => $type->id,
                                    'label' => $type->name,
                                    'selected' => $currentTypeScrapId == $type->id,
                                ]))"
                        />
                        @error('type_scrap_id')
                            <div class="field-error">{{ $message }}</div>
