...
@section('title', 'Crear Scrap')

@section('content_header')
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
        <div>
            <h1 class="m-0 font-weight-bold text-dark" style="font-size: 1.4rem;">Crear Scrap</h1>
        </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-6 mb-3 mb-md-0">
                        ...
                        <input type="text" name="code" id="code"
                            ...
                            value="{{ old('code') }}" required placeholder="Ej: SCR-001">
                        @error('code')
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="name" id="name"
                            ...
                            value="{{ old('name') }}" required placeholder="Ej: Scrap de Plástico ABS">
                        @error('name')
                            <div class="field-error">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        <x-select
                            name="type_scrap_id"
                            label="Tipo de Scrap *"
                            :options="collect([['value' => '', 'label' => 'Seleccione un tipo', 'selected' => !old('type_scrap_id')]])
                                ->concat($typeScraps->map(fn ($type) => [
                                    'value' => $type->id,
                                    'label' => $type->name,
                                    'selected' => old('type_scrap_id') == $type->id,
                                ]))"
                        />
                        @error('type_scrap_id')
                            <div class="field-error">{{ $message }}</div>
                ...
                <div class="row mb-3">
                    <div class="col-md-12">
                        ...
                        <textarea name="description" id="description" rows="3"
                            ...
                            placeholder="Descripción detallada del scrap">{{ old('description') }}</textarea>
                        @error('description')
                            <div class="field-error">{{ $message }}</div>
                        @enderror
                    </div>
