            ...
            <div class="space-y-4">
                ...
                @foreach ($areasData as $area)
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 border border-gray-100 dark:border-gray-700">
                        <h2 class="text-2xl font-bold mb-6 text-gray-800 dark:text-white border-b-2 border-blue-200 dark:border-blue-500 pb-3">
                            {{ $area['name'] }}
                        </h2>
                            ...
                            @foreach ($area['lines'] as $line)
                                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 border border-gray-200 dark:border-gray-600">
                                    <div class="flex items-center mb-4">
                                        ...
                                        <h3 class="text-lg font-semibold text-gray-800 dark:text-white">{{ $line['name'] }}</h3>
                                    </div>
                                            ...
                                            <a href="{{ route('guest.production-records', $workCenter['id']) }}"
                                                ...
                                                <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 border border-gray-200 dark:border-gray-600 hover:shadow-lg dark:hover:shadow-xl transition-shadow h-full">
                                                    <div class="mb-2 flex items-start justify-between">
                                                        <h4 class="font-medium text-gray-800 dark:text-white text-sm">{{ $workCenter['name'] }}</h4>
                                                        <div class="text-sm font-bold ml-2
                                                            @if($workCenter['percentage'] >= 90 && $workCenter['percentage'] <= 100)
                                                    ...
                                                    <div class="mt-3 grid grid-cols-2 gap-2 text-xs">
                                                        <div class="bg-blue-50 dark:bg-blue-900/50 p-2 rounded text-center border dark:border-blue-800">
                                                            ...
                                                            <div class="font-bold text-gray-800 dark:text-white">{{ $workCenter['planned'] ?? 0 }}</div>
                                                        </div>
                                                        <div class="bg-green-50 dark:bg-green-900/50 p-2 rounded text-center border dark:border-green-800">
                                                            ...
                                                            <div class="font-bold text-gray-800 dark:text-white">{{ $workCenter['total'] ?? 0 }}</div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </a>
    ...
    @script
        <script>
            Alpine.data('workCenterDashboard', () => {
                ...
                return {
                    init() {
                        // Inicializar las gráficas después de que el DOM esté listo
                        this.$nextTick(() => {
                        ...
                        this.$wire.on('refresh-production-records', () => {
                            setTimeout(() => {
                    ...
                    initializeAllCharts() {
                        const canvases = document.querySelectorAll('canvas[id^="wc-chart-"]');

                        canvases.forEach(canvas => {
                            const wcId = canvas.dataset.wcId;
                            const planned = parseInt(canvas.dataset.planned) || 0;
                            const produced = parseInt(canvas.dataset.produced) || 0;
                            const unplanned = parseInt(canvas.dataset.unplanned) || 0;
                    ...
                    updateAllCharts() {
                        // Destruir gráficas que ya no existen
                        const existingCanvases = new Set(
                            Array.from(document.querySelectorAll('canvas[id^="wc-chart-"]'))
                                .map(c => c.id)
                        );

                        charts.forEach((chart, canvasId) => {
                            if (!existingCanvases.has(canvasId)) {
                                chart.destroy();
                                charts.delete(canvasId);
                            }
                        });

                        // Actualizar o crear nuevas gráficas
                        const canvases = document.querySelectorAll('canvas[id^="wc-chart-"]');
                        ...
                        canvases.forEach(canvas => {
                            ...
                            if (charts.has(canvas.id)) {
                                this.updateChart(canvas.id, workCenter);
                            } else {
                                this.createChart(canvas.id, workCenter);
                            }
                        });
                    },

                    createChart(canvasId, workCenter) {
                    ...
                    updateChart(canvasId, workCenter) {
                        ...
                        // Actualizar solo los datos, no recrear la gráfica
                        chart.data.datasets[0].data = [workCenter.planned, 0];
                        chart.data.datasets[1].data = [0, workCenter.produced];
                        chart.data.datasets[2].data = [0, workCenter.unplanned];
