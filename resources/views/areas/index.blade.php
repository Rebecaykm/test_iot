                    ...
                    <form method="GET" action="{{ route('areas.index') }}">
                        <div class="input-group">
                            ...
                            @if (!empty(request('search')))
                                <a href="{{ route('areas.index') }}" class="input-group-text bg-white border-start-0">
                    ...
                    <tbody>
                        @forelse ($areas as $area)
                            <tr class="border-light-subtle">
                                ...
                                <!-- Nombre -->
                                <td class="py-3 fw-500">{{ $area->name }}</td>

                                <!-- Descripción -->
                                <td class="py-3">
                                    <span class="text-muted small">{{ $area->description ?? '-' }}</span>
                                </td>

                                <!-- Fecha de Creación -->
                                <td class="py-3 small">
                                    <div class="fw-500">{{ optional($area->created_at)->format('Y-m-d') }}</div>
                                    <div class="text-muted">{{ optional($area->created_at)->format('H:i') }}</div>
                                </td>

                                <!-- Fecha de Actualización -->
                                <td class="py-3 small">
                                    <div class="fw-500">{{ optional($area->updated_at)->format('Y-m-d') }}</div>
                                    <div class="text-muted">{{ optional($area->updated_at)->format('H:i') }}</div>
                                </td>
                                ...
                                <td class="py-3 text-center">
                                    <div class="d-flex justify-content-center gap-2">
                                        @can('edit areas')
                                            <a href="{{ route('areas.edit', $area->id) }}" class="btn btn-sm btn-outline-primary rounded-3">
                                        ...
                                        @can('delete areas')
                                            <form action="{{ route('areas.destroy', $area->id) }}" method="POST" style="display:inline;" class="delete-form">
                                                @csrf
                                                @method('DELETE')
            ...
            <div class="card-footer bg-white border-0 py-3">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="text-muted small">
                        Mostrando {{ $areas->firstItem() ?? 0 }} a {{ $areas->lastItem() ?? 0 }} de
