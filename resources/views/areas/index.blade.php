                    ...
                    <tbody>
                        @forelse ($areas as $area)
                            <tr class="td-row">
                                ...
                                <td class="td-cell">
                                    <span class="fw-600 text-dark" style="font-size: 0.85rem;">{{ $area->name }}</span>
                                </td>

                                <td class="td-cell">
                                    <span class="text-muted" style="font-size: 0.8rem;">{{ $area->description ?? '-' }}</span>
                                </td>
                                ...
                                <td class="td-cell">
                                    ...
                                    <div class="text-muted" style="font-size: 0.75rem;">
                                        {{ optional($area->created_at)->format('H:i') }}
                                    </div>
                                </td>

                                <td class="td-cell">
                                    <div class="fw-600" style="font-size: 0.82rem; color: #334155;">
                                        {{ optional($area->updated_at)->format('Y-m-d') }}
                                    </div>
                                    <div class="text-muted" style="font-size: 0.75rem;">
                                        {{ optional($area->updated_at)->format('H:i') }}
                                    </div>
                                </td>
            ...
            <div class="card-footer bg-white py-3" style="border-top: 1px solid #e9ecef;">
                <div class="d-flex justify-content-between align-items-center flex-wrap" style="gap: 0.5rem;">
                    <small class="text-muted">
                        ...
                        <strong>{{ $areas->total() }}</strong> registros
                    </small>
                    @if ($areas->hasPages())
...
@section('js')
    ...
    @include('partials.theme-scripts', ['deleteMessage' => '¿Estás seguro de que deseas eliminar esta área? Esta acción no se puede deshacer.'])
@stop
