...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
        <div>
            <h1 class="m-0 font-weight-bold text-dark" style="font-size: 1.4rem;">Editar Registro de Paro de Línea</h1>
        </div>
    </div>
@stop
        ...
        <div class="card-body p-4">
            <form action="{{ route('line-stoppage-records.update', $lineStoppageRecord->id) }}" method="POST"
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="minutes_stoppage" id="minutes_stoppage"
                            class="field-input @error('minutes_stoppage') is-invalid @enderror"
                            value="{{ old('minutes_stoppage', $lineStoppageRecord->minutes_stoppage) }}"
                            placeholder="Se calculará automáticamente" min="1" readonly>
                        <small class="field-hint">Se calculará automáticamente</small>
                ...
                <div class="d-flex justify-content-end gap-2 mt-4">
                    <a href="{{ route('line-stoppage-records.index') }}" class="btn-action btn-action-secondary">
    ...
    <script>
        $(document).ready(function () {
            ...
            function calculateMinutes() {
                ...
                if (!start || !end) { $('#minutes_stoppage').val(''); return; }

                const startDate = new Date(start);
                const endDate = new Date(end);
                if (endDate <= startDate) {
                    $('#minutes_stoppage').val('');
                    Swal.fire({ icon: 'error', title: 'Error', text: 'La hora de fin debe ser posterior a la hora de inicio.', toast: true, position: 'top-end', showConfirmButton: false, timer: 3000 });
                    return;
                }

                const diffMinutes = Math.floor((endDate - startDate) / 60000);
                $('#minutes_stoppage').val(diffMinutes < 1 ? '' : diffMinutes);
            }
