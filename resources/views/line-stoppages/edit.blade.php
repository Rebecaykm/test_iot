...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
        <div>
            <h1 class="m-0 font-weight-bold text-dark" style="font-size: 1.4rem;">Editar Paro de Línea</h1>
        </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
        <div class="card-header bg-white py-3" style="border-bottom: 1px solid #e9ecef;">
            <h5 class="mb-0 section-title">
                <i class="fas fa-ban mr-2" style="color: #94a3b8;"></i>Información del Paro de Línea
            </h5>
        </div>
        <div class="card-body p-4">
            <form action="{{ route('line-stoppages.update', $lineStoppage->id) }}" method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        @php $currentTypeLineStoppageId = old('type_line_stoppage_id', $lineStoppage->type_line_stoppage_id); @endphp
                        <x-select
                            name="type_line_stoppage_id"
                            label="Tipo de Paro *"
                            :options="collect([['value' => '', 'label' => 'Seleccione un tipo', 'selected' => !$currentTypeLineStoppageId]])
                                ->concat($typeLineStoppages->map(fn ($type) => [
                                    'value' => $type->id,
                                    'label' => $type->name,
                                    'selected' => $currentTypeLineStoppageId == $type->id,
                                ]))"
                        />
                ...
                <div class="row mb-3">
                    <div class="col-md-12">
                        ...
                        <textarea name="description" id="description" rows="3"
                            ...
                            placeholder="Descripción detallada del paro de línea">{{ old('description', $lineStoppage->description) }}</textarea>
                        @error('description')
                            <div class="field-error">{{ $message }}</div>
                        @enderror
                    </div>
